...
;(async () => {
  ...
  try {
    ...
    await page.getByLabel('Usuario de acceso', { exact: true }).fill('nueva.miembro')
    await page.getByLabel('Contraseña', { exact: true }).fill('PruebaSegura456')
    await page.getByRole('button', { name: 'Iniciar sesión', exact: true }).click()
    await page.waitForURL(`${base}/`)
    assert.equal(credentials.tipo_acceso, 'usuario')
    assert.equal(credentials.identificador, 'nueva.miembro')
    console.log('Ingreso de miembro por usuario sin datos de contacto: OK')
    await context.close()
