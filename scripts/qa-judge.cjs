...
async function checkMatchEditing(browser) {
  ...
  try {
    ...
    for (const index of [0, 1, 0]) {
      ...
      const first = modal.locator('[name="equipo1_id"]')
      const second = modal.locator('[name="equipo2_id"]')
      assert.equal(await first.locator(`option[value="${matches[index].equipo2.id}"]`).count(), 0)
      assert.equal(await second.locator(`option[value="${matches[index].equipo1.id}"]`).count(), 0)
      await first.selectOption('')
      assert.equal(await second.locator(`option[value="${matches[index].equipo1.id}"]`).count(), 1)
      await first.selectOption(String(matches[index].equipo1.id))
...
;(async () => {
  ...
  try {
    ...
    assert.ok(page.url().endsWith('/sponsors'), 'Officials can access public pages when also acting as players')
