...
test('un set completo largo no se convierte en supertiebreak', () => {
  const stats = calculatePlayerStats([row({ partido: 99, jugador1: 1, jugador2: 2,
    ganador: 'jugador1', set: 3, games1: 12, games2: 10, decidingSet: 'set_completo' })])
  assert.equal(stats.find(p => p.jugador_id === 1).games_ganados, 12)
  assert.equal(stats.find(p => p.jugador_id === 2).games_ganados, 10)
})
